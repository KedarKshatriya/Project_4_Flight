...
contract('Flight Surety Tests', async (accounts) => {
    ...
    it(`(multiparty) can block access to setOperatingStatus() for non-Contract Owner account`, async function () {
        ...
        try {
            await config.flightSuretyData.setOperatingStatus(false, {from: config.testAddresses[80]});
        } catch (e) {
            accessDenied = true;
        }
    ...
    it("Passengers may pay up to 1 ether for purchasing flight insurance.", async () => {
            let airline = accounts[42];
            let customer = accounts[33];
            let timestamp = 42345678;
